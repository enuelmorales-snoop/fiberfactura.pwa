...
var stateRenderPdf = undefined


function renderPage(pageNum,page){
  try{
   var valor = 'page'+ (pageNum + 1) ;
   var canvas =  document.getElementById(valor);
...
function render() {
  ...
  try{
      if (stateRenderPdf === undefined){
          stateRenderPdf = 'render';
          var numPages = PDFViewerApplication.pagesCount;
          for (var pageNum = 1; pageNum <= numPages; pageNum++) {
              var  px = PDFViewerApplication.pdfDocument.getPage(pageNum);
              pagePromises.push(px);
          }

          Promise.all(pagePromises).then(function(pages){
            for (var num = 0; num < pages.length; num++) {
              var page = pages[num];
              renderPromises.push(renderPage(num,page));
            }

            Promise.all(renderPromises).then(function(){
              stateRenderPdf = undefined;
            });    
          }); 
      }
  }catch(err){
...
Libreria de JS que permite detectar gesture en el mobile. Sólo estoy detectando cuando hace zoom in / zoom out
y luego se llama a una función de JS que renderiza el PDF para no perder calidad.
**/
function initializeHammerJS() {
var elm = document.getElementById('viewerContainer');
...
hammertime.on('doubletap pan pinch panend pinchend', function(ev) {
    ...
    if(ev.type == "pinchend"){
      ...
      if (last_scale === scale){ //Maximo Zoom
          return;
      }
...
function shareFacturaPdf(){
  showLoading('Cargando...');
  window.location.href = '#sharePDF';
}

function showLoading(text){
  document.getElementById('idShowInfoDetail').innerHTML = text;
...
function showInfo(text){
  document.getElementById('alert_info_contenido').innerHTML = text;
...
function showMessageAndroid(){
  showInfo("Visualizá tu factura en tu carpeta de descargas.");
}

function showMessageIOS(){
  showInfo("Ahora podes acceder a la factura sin estar conectado a una red wifi.");
}
...
var PDFPageView = (function PDFPageViewClosure() {
  ...
  function PDFPageView(options) {
    ...
    div.style.height = (Math.floor(this.viewport.height)+50 ) + 'px';
  ...
  PDFPageView.prototype = {
    ...
    reset: function PDFPageView_reset(keepZoomLayer, keepAnnotations) {
      ...
      var div = this.div;
      div.style.width = Math.floor(this.viewport.width-30) + 'px';
      div.style.height = Math.floor(this.viewport.height-30) + 'px';
    ...
    draw: function PDFPageView_draw() {
      ...
      canvasWrapper.style.height = div.style.height + 40;
      ...
      canvas.width = roundToDivide(viewport.width * outputScale.sx + 80, sfx[0]);
      canvas.height = roundToDivide(viewport.height * outputScale.sy, sfy[0]);
      canvas.style.width = '105%'; //roundToDivide(viewport.width, sfx[1]) + 'px';
    ...
    beforePrint: function PDFPageView_beforePrint() {
      ...
      canvasWrapper.style.height = (viewport.height + 40) + 'pt';
      canvasWrapper.appendChild(canvas);
